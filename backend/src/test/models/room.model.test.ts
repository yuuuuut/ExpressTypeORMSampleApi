import { createFirebaseUser, createTestRoom, createTestUser, getTestUser } from '@/test/common'
import * as model from '@/model/room.model'
...
describe('Room Model Test', () => {
  describe('index Test', () => {
    it('戻り値が正しいこと。', async () => {
      ...
      const val = await model.index(testCurrentUser.id)
  ...
  describe('show Test', () => {
    it('戻り値が正しいこと。', async () => {
      ...
      const val = await model.show(testRoom.id, testCurrentUser.id)
  ...
  describe('isRoomBool Test', () => {
    it('Roomが存在する場合の戻り値が正しいこと。', async () => {
      ...
      const val = model.isRoomBool(user, currentUser)
  ...
  describe('isUserBelongstoRoom Test', () => {
    it('正しく動作すること。', async () => {
      ...
      expect(() => model.__local__.isUserBelongstoRoom([testCurrentUser, testUser], testCurrentUser.id)).not.toThrow()
    })
    it('正しく動作すること。', async () => {
      const testCurrentUser = await createFirebaseUser()
      const testUser = await createTestUser()

      expect(() => model.__local__.isUserBelongstoRoom([testCurrentUser, testUser], testUser.id)).not.toThrow()
    })
    it('Roomに所属しないUserだった場合、Errorが発生すること', async () => {
      ...
      expect(() => model.__local__.isUserBelongstoRoom([testCurrentUser, testUser1], testUser2.id)).toThrow(
        '許可されていない操作です。'
      )
    })
  })
